  ...
  const getNavigationItems = (): NavigationItem[] => {
    ...
    if (userRole === 'admin' || userRole === 'superadmin') {
      return [
        ...
        // Only /admin/contracts itself has a real backend (fetches from a
        // live API) - the 7 sub-pages this dropdown used to link to
        // (generator/templates/signing/preview/archive/analytics/content)
        // are all prototype code with no backend at all (grepped every one
        // for a fetch() call - zero matches), and contract-analytics in
        // particular rendered entirely fabricated numbers (fake revenue,
        // fake named "top performers") that then crashed at runtime anyway.
        // Removed the dropdown so nobody can reach them from navigation;
        // the page files are left in place in case this becomes a real,
        // built-out feature later.
        { id: 'contracts', label: 'Contracts', icon: '📄', href: '/admin/contracts' },
