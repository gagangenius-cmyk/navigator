...
export async function GET(request: NextRequest, context: RouteContext) {
  ...
  try {
    const contact = await ClientPortalProductService.getContactCpo(client.leadId, Number(opportunityId));
    return NextResponse.json(contact);
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Failed to load contact information';
