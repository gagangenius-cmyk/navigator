...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (action === 'test') {
      ...
      const [leads] = await sequelize.query('SELECT * FROM crm_forum_leads LIMIT 10');
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const [employeeResult] = await sequelize.query('SELECT id FROM crm_employee LIMIT 1');
    const [branchResult] = await sequelize.query('SELECT id FROM crm_branch LIMIT 1');
    const [regionResult] = await sequelize.query('SELECT id FROM crm_region LIMIT 1');
