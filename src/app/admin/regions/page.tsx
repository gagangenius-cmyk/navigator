...
export default function RegionsManagement() {
  ...
  const [selectedRegion, setSelectedRegion] = useState<CrmRegionAttributes | null>(null);
  ...
  const handleViewRegion = (region: CrmRegionAttributes) => {
    setSelectedRegion(region);
    setShowModal(true);
  };

  const handleEditRegion = (region: CrmRegionAttributes) => {
    setSelectedRegion(region);
  ...
  const handleAddRegion = async (regionData: Partial<CrmRegionAttributes>) => {
    try {
  ...
  const handleUpdateRegion = async (regionData: Partial<CrmRegionAttributes>) => {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow p-4">
        ...
        <SortButtonRow
          ...
          onSort={toggleRegionSort}
        />
        <RecordList isEmpty={false}>
          {sortedRegions.map((region: CrmRegionAttributes) => (
            <RecordCard
              key={region.id}
              avatar={
                <input
              ...
              title={<span className="min-w-0 break-words text-base font-bold text-gray-950">{region.name}</span>}
              titleBadges={
              ...
              actions={[
                { key: 'view', icon: Eye, label: 'View', onClick: () => handleViewRegion(region) },
                { key: 'edit', icon: Pencil, label: 'Edit', onClick: () => handleEditRegion(region) },
                { key: 'delete', icon: Trash2, label: 'Delete', onClick: () => handleDeleteRegion(region.id), colorClass: 'bg-red-50 text-red-700 hover:bg-red-100', hidden: !canDelete },
              ]}
            />
          ))}
        </RecordList>
...
interface RegionFormModalProps {
  title: string;
  initialData?: CrmRegionAttributes | null;
  onSubmit: (data: Partial<CrmRegionAttributes>) => void;
  onClose: () => void;
}
